...
describe('CrowdFunding', () => {
  ...
  it('withdraw success', async () => {
    ...
    await txn.prove();
    await txn.sign([deployerKey, zkAppPrivateKey]).send();
    expect(zkApp.account.balance.get()).toEqual(UInt64.from(80));

    await expect(async () => {
      await Mina.transaction(deployerAccount, async () => {
        await zkApp.withdraw();
      })
    }).rejects.toThrow('Withdrawal not allowed');

    for (let i = 1; i <= 8; i++) {
      console.log(`after every 200 block to withdraw ${i}, next block ${100 + i * 200}`);
      Local.setBlockchainLength(UInt32.from(100 + i * 200));
      txn = await Mina.transaction(deployerAccount, async () => {
        await zkApp.withdraw();
      });
      await txn.prove();
      await txn.sign([deployerKey, zkAppPrivateKey]).send();
      expect(zkApp.account.balance.get()).toEqual(UInt64.from(80 - i * 10));
    }
    expect(zkApp.account.balance.get()).toEqual(UInt64.from(0));
    ...
    }).rejects.toThrow('No balance to withdraw');
  })
});
